 ...
 */
public class MyHomeRecycerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if(viewType == 0){
            ...
            return new MyAdViewHolder(itemView);
        }
        else {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof MyViewHolder) {
            ...
            ((MyViewHolder) holder).user_name.setText(item.getUserName());
            ((MyViewHolder) holder).total_comment.setText(item.getAtyComment());
            ((MyViewHolder) holder).aty_name.setText(item.getAtyName());
            ...
            if (!"001".equals(MyUser.userId)) {
                ((MyViewHolder) holder).imageViewContainer.setVisibility(View.GONE);
            } else if (item.getAtyIsJoined().equals("false") && item.getAtyIsPublic().equals("")) {

            }

            ((MyViewHolder) holder).imageViewContainer.removeAllViews();
            if (item.getAtyAlbum() != null) {
                ...
                for (int i = 0; i < item.getAtyAlbum().size(); i++) {
                    ...
                    ((MyViewHolder) holder).imageViewContainer.addView(imageView);
                }
            }

            if (item.getAtyPlused().equals("false")) {
                ((MyViewHolder) holder).plus_fab.setBackgroundTintList(ColorStateList.valueOf(activity.getResources().getColor(R.color.fab_gray)));
                ((MyViewHolder) holder).plus_fab.setImageDrawable(activity.getResources().getDrawable(R.drawable.ic_action_plus_one));
            } else if (item.getAtyPlused().equals("true")) {
                ((MyViewHolder) holder).plus_fab.setBackgroundTintList(ColorStateList.valueOf(activity.getResources().getColor(R.color.primary)));
                ((MyViewHolder) holder).plus_fab.setImageDrawable(activity.getResources().getDrawable(R.drawable.ic_action_plus_one_white));
            }

            if (item.getAtyJoined().equals("true")) {
                ((MyViewHolder) holder).mjoinBtn.setText("已加入");
                ((MyViewHolder) holder).mjoinBtn.setTextColor(activity.getResources().getColor(R.color.primary));
            } else if (item.getAtyJoined().equals("false")) {
                ((MyViewHolder) holder).mjoinBtn.setText("加入");
                ((MyViewHolder) holder).mjoinBtn.setTextColor(activity.getResources().getColor(R.color.black));
            }

            setAnimation(((MyViewHolder) holder).cardView, position-1);

        }else if(holder instanceof MyAdViewHolder){
            WindowManager windowManager = activity.getWindowManager();
            DisplayMetrics dm = new DisplayMetrics();
            Display display = windowManager.getDefaultDisplay();
            int width = display.getWidth() - 7;
            int height = display.getHeight();
            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(width, height * 2 / 9);
            for (int i=0;i<images.length;i++){
                ImageView iv=new ImageView(activity);
                iv.setLayoutParams(layoutParams);
                Picasso.with(activity).load(images[i]).into(iv);
                iv.setScaleType(ImageView.ScaleType.CENTER_CROP);
                ((MyAdViewHolder)holder).flipper.addView(iv);
            }
            ((MyAdViewHolder)holder).flipper.setAutoStart(true);
            ((MyAdViewHolder)holder).flipper.setFlipInterval(3000);
            if(((MyAdViewHolder)holder).flipper.isAutoStart()&&!((MyAdViewHolder)holder).flipper.isFlipping()){
                ((MyAdViewHolder)holder).flipper.startFlipping();
            }
        }
    }
    ...
    public static class MyAdViewHolder extends RecyclerView.ViewHolder{

        @Bind(R.id.view_flipper)ViewFlipper flipper;
    ...
    public static class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            if (v instanceof CircleImageView) {
                ...
                mListener.onUserPhoto((CircleImageView)v,getAdapterPosition()-1);
            } else if ((v instanceof FloatingActionButton) && (v.getId() == R.id.fab_comment)) {
                mListener.onComment((FloatingActionButton) v, getAdapterPosition()-1);
            } else if ((v instanceof FloatingActionButton) && (v.getId() == R.id.fab_plus)) {
                mListener.onPlus((FloatingActionButton) v, getAdapterPosition()-1, totle_plus);
            } else if (v instanceof ImageView) {
                mListener.onPicture((ImageView) v,getAdapterPosition()-1);
            } else if (v instanceof CardView) {
                mListener.onCard((CardView) v, getLayoutPosition()-1);
            } else if (v instanceof Button) {
                mListener.onJoinBtn((Button) v, getAdapterPosition()-1);
            }else if(v instanceof TextView && (v.getId() == R.id.community_name)){
                mListener.onCommunity((TextView)v,getAdapterPosition()-1);
            }
        }
